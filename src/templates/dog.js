...
import { Container, Row, Col, Table, Badge, Alert, Button, Tooltip } from 'reactstrap'
...
function DogPage(props) {
    ...
    useEffect(() => {
        ...
        for (let language in props.data.dogdata.frontmatter.lang) {
            remark().use(html).process(props.data.dogdata.frontmatter.lang[language], (err, file) => {
        ...
        <Layout>
            <Container style={{ paddingTop: '6rem' }}>
                <Row className="d-flex align-items-center">
                    <Col xs={{size: 12, order: 'last'}} sm={{size: 6, order: 'first'}}>
                        <h1 className="display-3 font-weight-bold mb-0">{frontmatter.name}</h1>
                    </Col>
                ...
                <div className="pb-2">
                    ...
                    {
                        frontmatter.ppp ? (
                            <Badge color="warning" className="p-1 ml-1" id="ppp-tooltip" onMouseEnter={toggleTooltip} onMouseLeave={toggleTooltip} onTouchStart={()=>{
                                if(!tooltipOpen){
                                    setTooltipOpen(true)
                                }
                            }} onTouchEnd={()=>{
                                ...
                                <Tooltip placement="bottom" fade={false} isOpen={tooltipOpen} target="ppp-tooltip">Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur molestiae praesentium, alias porro assumenda repudiandae ipsum sed ea ullam perspiciatis.</Tooltip>
                            </Badge>
                        ) : null
                    }
                </div>
                ...
                <Row>
                    <Col>
                        {biog ? (<div dangerouslySetInnerHTML={{ __html: biog[props.lang] }} className="pt-3" />) : null}
                    </Col>
                </Row>
                <Row>
